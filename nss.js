...
class RouteManager{
    ...
    //<script src="url 2 nss.js"></script>
    //<script type="module">
...
class App{
    constructor(repo,branch="main",options={"fspt":"files","tmplpt":"templates"}){
        this.repo = repo
        this.branch = branch
        this.fm = new FileManager(repo,branch,options.fspt)
        this.routes = new RouteManager(this.fm)
        this.template = new TemplateManager(repo,branch,options.tmplpt)
        this.errors = {}


    }
    route(route,cb){
        this.routes.add(Route(route,cb))
    }
    on(error){
        //404 + func error
    }
